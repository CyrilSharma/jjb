    ...
    match root {
        ...
        Tree::LetC(ClassDeclaration {
            ...
            for method in methods {
                traverse(&method, state)
            }
        }
        Tree::LetE(_) => todo!(),
        Tree::LetP(PrimStatement { name, typ, exp }) => {
            if let Some(n) = name {
                inc(&mut state.var_syms, *n)
            }
...
fn operand(op: &Operand, state: &mut CheckState) {
    match op {
        ...
        Operand::T(ExprTree { op, args }) => match op {
            ...
            _ => {
                args.iter().for_each(|arg| operand(arg, state));
            }
        },
        other => todo!(),
    }
...
"#,
    |c, _| { assert_eq!(c.label_syms.keys().len(), 6) }
);
