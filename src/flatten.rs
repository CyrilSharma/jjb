...
pub fn flatten(tree: Tree, sm: &mut SymbolManager) -> Tree {
    ...
    match tree {
        Tree::Program(stmts) => Tree::Program(stmts.into_iter().map(|f| flatten(f, sm)).collect()),
        Tree::LetI(i) => Tree::LetI(i),
        Tree::LetF(f) => Tree::LetF({
            let mut body = f.body;
            for _ in 0..5 {
                body = flattenlist(body, &mut census, sm)
            }
            FunDeclaration {
                body,
                name: f.name,
    ...
    while let Some(mut item) = list.pop_front() {
        match item {
            ...
            Tree::Switch(SwitchStatement {
                ...
                res.push_back(Tree::Switch(SwitchStatement {
                    arg,
                    label,
                    cases: newcases,
                    default,
            ...
            Tree::If(IfStatement {
                ...
                res.push_back(Tree::If(IfStatement {
                    cond,
                    label,
                    btrue,
                    bfalse,
...
fn inline(e: &mut Operand, census: &mut HashMap<Symbol, usize>, values: &HashMap<Symbol, Operand>) {
    match e {
