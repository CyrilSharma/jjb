    ...
    match root {
        ...
        Tree::LetC(classdecl) => {
            ...
            for s in &classdecl.methods {
                methods.extend(statement(&s, state))
            }
        ...
        Tree::Block(bstmt) => {
            ...
            for s in &bstmt.bbody {
                bbody.extend(statement(&s, state))
            }
        ...
        Tree::Switch(sstmt) => {
            ...
            for (ops, code) in &sstmt.cases {
                ...
                for s in code {
                    newcode.extend(statement(&s, state))
                }
        ...
        Tree::Loop(lstmt) => {
            ...
            if lstmt.dowhile {
                ...
                lstmt
                    .lbody
                    .iter()
                    .for_each(|s| bbody.extend(statement(&s, state)));
                ...
                lstmt
                    .lbody
                    .iter()
                    .for_each(|s| lbody.extend(statement(&s, state)));
            }
        ...
        Tree::If(IfStatement {
            ...
            btrue
                .iter()
                .for_each(|s| nbtrue.extend(statement(s, state)));
            bfalse
                .iter()
                .for_each(|s| nbfalse.extend(statement(s, state)));
            head.push_back(Tree::If(IfStatement {
                cond,
                label: *label,
                btrue: nbtrue,
