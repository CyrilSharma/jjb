...
impl State {
    ...
    pub fn use_label(&mut self, label: Symbol, body: Id, next: Id) {
        self.label_map
            .insert(label, body)
            .map(|s| panic!("Overwriting label: {:?}", label));
        self.break_map
            .insert(label, next)
            .map(|s| panic!("Overwriting label: {:?}", label));
    }
    pub fn continue_id_from_label(&self, label: Symbol) -> Id {
        *self
            .label_map
            .get(&label)
            .expect(&format!("Label {:?} does not exist in map", label))
    }
    ...
    pub fn add_children(&mut self, node: &mut CfgNode, children: &[Id]) {
        ...
        for &child in children {
            self.alloc.add_child(node.id, child)
        }
    }
    pub fn connect(&mut self, ids: &[Id], target: Id) {
        for &child in ids.as_ref() {
            self.alloc.add_child(child, target)
        }
    }
}
    ...
    while let Some(stmt) = cur {
        ...
        match stmt {
            ...
            Tree::Continue(label) => {
                ...
                state
                    .alloc
                    .set(nid, content, vec![state.continue_id_from_label(label)]);
                return (nid, tails.into_boxed_slice());
            }
            Tree::Break(label) => {
                ...
                state
                    .alloc
                    .set(nid, content, vec![state.break_id_from_label(label)]);
                return (nid, tails.into_boxed_slice());
            }
...
fn fold_graph(id: Id, allocator: &mut Allocator, next_map: &HashMap<Id, Id>) -> TreeContainer {
    ...
    if let Some(n) = next_map.get(&id) {
        content.append(&mut fold_graph(*n, allocator, next_map));
    }
    content
}
...
pub fn dominance_frontier(doms: &Vec<Id>, alloc: &Allocator) -> Vec<FixedBitSet> {
    let mut df: Vec<FixedBitSet> = vec![FixedBitSet::with_capacity(alloc.len()); alloc.len()];
...
pub fn stat(alloc: &Allocator) -> (HashMap<Symbol, HashSet<usize>>, HashMap<Symbol, Typ>) {
...
pub fn used(
    alloc: &mut Allocator,
    sm: &SymbolManager,
) -> (Vec<HashSet<Symbol>>, Vec<HashSet<Symbol>>) {
        ...
        for item in node.content.iter().rev() {
            match item {
                ...
                Tree::LetP(p) => {
                    ...
                    if let Some(e) = p.exp.as_ref() {
                        useop(e, &mut uses[i], sm)
                    }
                }
...
pub fn useop(e: &Operand, mp: &mut HashSet<Symbol>, sm: &SymbolManager) {
    match e {
        Operand::This(_) | Operand::Super(_) | Operand::C(_) | Operand::Tp(_) => (),
        Operand::V(sym) => {
            mp.insert(*sym);
        }
        Operand::T(ExprTree {
        ...
        Operand::T(ExprTree {
            args,
            op: Operation::InvokeVirtual | Operation::InvokeStatic,
        }) => {
            useop(&args[0], mp, sm);
            args[2..].iter().for_each(|e| useop(e, mp, sm))
        }
        Operand::T(ExprTree { args, .. }) => args.iter().for_each(|e| useop(e, mp, sm)),
        Operand::A(a) => match a {
